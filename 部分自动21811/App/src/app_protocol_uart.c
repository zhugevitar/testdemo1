...
static INT16U app_protocol_uart_crc16( INT8U *ptr, INT16U len )
{
    ...
    while ( len-- != 0 )
    {
        da = ( INT8U )(crc >> 8);     //以8位二进制数的形式暂存CRC的高8位
...
//      Dat：有效数据
// 返回：无
// 说明：串口协议包发送的时候必须连续发送，不能间断，多方接收到长时间间断后会认为数据包结束
//============================================================================
void app_protocol_uart_send_msg( INT8U DeviceType, INT8U Address, INT8U Cmd, INT8U *Dat, INT8U Len )
{
    INT8U   SendBuffer[100];
    INT8U   i = 0, j;
    INT16U  Crc16Value;

    SendBuffer[i++] = UART_MSG_PACKAGE_STARTCHAR;
    SendBuffer[i++] = DeviceType;
    SendBuffer[i++] = Address;
    SendBuffer[i++] = Cmd;
    SendBuffer[i++] = Len;
    for( j = 0; j < Len; j++ )
    {
